/*
 * This code successfully finds and displays the location of the small piece of reflective
 * tape on the piece of cardboard.
 *
 *
 *@author Ryan Young
 */
...
public class VisionTarget
{
	...
	public static VisionTarget findSecondTarget(ColorImage image,
			Threshold firstThreshold, VisionTarget firstTarget) throws NIVisionException
	{
		...
		for (int i = 0; i < secondColorHits.length; i++)
		{
			...
			//firstTrackReport.center_mass_y >= -.1 && firstTrackReport.center_mass_y <= .1  possible alternate filter
		    if ((target.getSize()) >= 1.0) //&& firstTarget.getSize() < target.getSize())
			{
							// add in the SizesRelative call if needed -
							// so far it does not seem necessary
			   return target;
